...
def get_trend_analysis_query():
    ...
    WHERE s.vehicle_id = %(vehicle_id)s
      AND sr.timestamp >= CURRENT_DATE - (%(days)s * INTERVAL '1 day')
    ORDER BY sr.timestamp;
    """
    return query

def perform_trend_analysis(vehicle_id, days=30):
    ...
    try:
        cursor = conn.cursor()
        query = get_trend_analysis_query()
        params = {'vehicle_id': vehicle_id, 'days': days}
        cursor.execute(query, params)
        column_names = [desc[0] for desc in cursor.description]
        results = cursor.fetchall()

        if not results:
            print(f"No trend data found for vehicle ID {vehicle_id} in the last {days} days.")
        else:
            df = pd.DataFrame(results, columns=column_names)
            print(f"\nTREND ANALYSIS FOR VEHICLE ID {vehicle_id} - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"Showing sensor readings for the last {days} days:\n")
...
if __name__ == "__main__":
    main()
    vehicle_id = 123  
    perform_trend_analysis(vehicle_id, days=30)