...
reliab AS (
  SELECT 
    ...
    AVG(measured_value - threshold) AS avg_reliability_margin
...
SELECT 
  ...
  r.avg_threshold,
  r.avg_reliability_margin
FROM maint m
FULL OUTER JOIN comp c ON m.aircraft_id = c.aircraft_id
FULL OUTER JOIN reliab r ON COALESCE(m.aircraft_id, c.aircraft_id) = r.aircraft_id
ORDER BY COALESCE(m.total_downtime, 0) DESC;
"""
