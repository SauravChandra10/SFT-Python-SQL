import psycopg2
import csv
import logging

def setup_logging():
    logging.basicConfig(
        filename='logs.txt',
        level=logging.INFO,
        format='%(asctime)s %(levelname)s: %(message)s',
        datefmt='%Y-%m-%d %H:%M:%S'
    )
    logging.info('Logging initialized.')

def run_query(connection, query):
    with connection.cursor() as cur:
        cur.execute(query)
        results = cur.fetchall()
        columns = [desc[0] for desc in cur.description]
    logging.info('Executed query: %s ...', query.strip().splitlines()[0])
    return columns, results

def save_to_csv(filename, columns, results):
    try:
        with open(filename, mode='w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(columns)
            writer.writerows(results)
        logging.info('Results successfully saved to %s', filename)
    except Exception as e:
        logging.error('Failed to write to %s: %s', filename, e)

def main():
    setup_logging()
    conn_params = {
        'dbname': 'metal_processing_db',
        'user': 'admin',
        'password': 'admin_password',
        'host': 'localhost',
        'port': 5432
    }

    try:
        conn = psycopg2.connect(**conn_params)
        logging.info('Connected to the database successfully.')
    except Exception as e:
        logging.error('Error connecting to the database: %s', e)
        return

    query1 = """
    SELECT 
        rm.metal_type,
        COUNT(DISTINCT pb.batch_id) AS completed_batches,
        AVG(EXTRACT(EPOCH FROM (pb.end_time - pb.start_time)) / 3600) AS avg_processing_time_hours,
        SUM(CASE WHEN ml.event_type = 'Error' THEN 1 ELSE 0 END) AS error_count
    FROM raw_materials rm
    JOIN processing_batches pb ON rm.material_id = pb.material_id
    LEFT JOIN machine_logs ml ON pb.batch_id = ml.batch_id
    WHERE pb.status = 'Completed'
    GROUP BY rm.metal_type;
    """

    query2 = """
    SELECT 
        rm.supplier,
        COUNT(pb.batch_id) AS total_batches,
        COUNT(CASE WHEN pb.status = 'Failed' THEN 1 END) AS failed_batches,
        ROUND(100.0 * COUNT(CASE WHEN pb.status = 'Failed' THEN 1 END) / COUNT(pb.batch_id), 2) AS failure_percentage,
        ROUND(AVG(EXTRACT(EPOCH FROM (pb.end_time - pb.start_time)) / 3600)::numeric, 2) AS avg_processing_hours
    FROM raw_materials rm
    JOIN processing_batches pb ON rm.material_id = pb.material_id
    WHERE pb.status IN ('Completed', 'Failed')
    GROUP BY rm.supplier;
    """

    try:
        columns1, results1 = run_query(conn, query1)
        save_to_csv('output1.csv', columns1, results1)
    except Exception as e:
        logging.error('Error executing Query 1: %s', e)

    try:
        columns2, results2 = run_query(conn, query2)
        save_to_csv('output2.csv', columns2, results2)
    except Exception as e:
        logging.error('Error executing Query 2: %s', e)

    conn.close()
    logging.info('Database connection closed.')

if __name__ == '__main__':
    main()